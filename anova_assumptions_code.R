...
### Create functions for assumption testing
...
### Custom function 
## Create DF with transformed value and residuals
transform_tester<-function(data,trans,test){
  data %>%
    mutate(trans_value=trans(value)) %>%
    lm(trans_value~trmt,.) %>%
    augment() %>%
    select(trmt,trans_value,resid=".resid") -> sampDF_trans
    #summarize(shapiro=shapiro_test(resid),
              #levene=levene_test(.,trans_value~trmt)) 
}

transform_tester(sampDF,log)

## Test normality graphically--TURN THIS INTO FUNCTION
mod_log_residDF %>%
  ggplot(aes(sample=resid)) +
  stat_qq(color="steelblue") + 
  stat_qq_line() +
